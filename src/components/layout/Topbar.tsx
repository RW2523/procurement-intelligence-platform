  ...
  return (
    <header className="h-16 shrink-0 bg-[var(--color-surface)] border-b border-[var(--color-border)] flex items-center gap-3 px-6">
      ...
      <div className="flex items-center gap-1.5 ml-auto">
        {/* The tray is procurement data, and the notifications table is not scoped
            per person — so it is drawn only for an actual procurement account.
            The real gate is getShellData(), which no longer fetches any of this
            without one; this guard keeps the component safe on its own terms, so
            a future caller cannot reintroduce the leak by passing rows in. */}
        {user && <NotificationBell unread={unread} notifications={notifications} />}
...
function NotificationBell({ unread, notifications }: { unread: number; notifications: Notification[] }) {
